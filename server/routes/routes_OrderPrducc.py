...
@OrdenProducc.get("/admin/order-production", tags=["Orden de Produccion"], dependencies=[Depends(Portador()),Depends(protectedAcountAdmin())], description="Devuelve todas las ordenes de produccion echas, acceso solo admin")
async def find_all_admin_orders():
    return Ordenes_ProduccionEntity(db_name.OrderProduction.find())

@OrdenProducc.get("/admin/order-production/status/pending", tags=["Orden de Produccion"], dependencies=[Depends(Portador()),Depends(protectedAcountAdmin())], description="Devuelve todas las ordenes de produccion en estado pendiente, acceso solo admin")
async def find_pendindg_admin_orders():
    return Ordenes_ProduccionEntity(db_name.OrderProduction.find({"status":"pending"}))

@OrdenProducc.get("/admin/order-production/status/complete", tags=["Orden de Produccion"], dependencies=[Depends(Portador()),Depends(protectedAcountAdmin())], description="Devuelve todas las ordenes de produccion en estado completado, solo acceso admin")
